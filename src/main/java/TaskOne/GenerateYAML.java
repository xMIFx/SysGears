...
public class GenerateYAML {
    public static void main(String[] args) throws IOException {
        ...
        int min = 0;
        int maxCountOfNumbers = 1000000;
        int max = Integer.MAX_VALUE / 2;

        int countOfNumbers = min + (int) (Math.random() * ((maxCountOfNumbers - min) + 1));
        // We creat array with even count of Numbers. All numbers have a pair. then we remove one.
        if (countOfNumbers % 2 != 0) {
            countOfNumbers = countOfNumbers + 1;
        }
        System.out.println(countOfNumbers);
        // I need to remove one element, so i use ArrayList, not array.
        ArrayList<Integer> array = new ArrayList<Integer>(countOfNumbers);
